...
# Final figure for non-interferon
up.peaks <- nrow(res.dt[ko4H3_v_YR.padj < 0.05 & ko4H3_v_YR.l2fc > 0])
down.peaks <- nrow(res.dt[ko4H3_v_YR.padj < 0.05 & ko4H3_v_YR.l2fc < 0])
mmvl30.up.peaks <- nrow(overlaps.mmvl30[ko4H3_v_YR.padj < 0.05 & ko4H3_v_YR.l2fc > 0])
mmvl30.down.peaks <- nrow(overlaps.mmvl30[ko4H3_v_YR.padj < 0.05 & ko4H3_v_YR.l2fc < 0])
mp <- ggplot(res.dt, aes(ko4H3_v_YR.l2fc, ko4H3_v_YR.neglogp)) +
  geom_vline(xintercept = 0, linetype="dashed", color = "gray") +
  geom_point(data = subset(res.dt, ko4H3_v_YR.padj >= 0.05), color = "gray", size = 0.2) +
  geom_point(data = subset(res.dt, ko4H3_v_YR.l2fc < 0 & ko4H3_v_YR.padj < 0.05), color = "blue", size = 0.2) +
  geom_point(data = subset(res.dt, ko4H3_v_YR.l2fc > 0 & ko4H3_v_YR.padj < 0.05), color = "red", size = 0.2) +
  # geom_point(data = subset(overlaps, !is.na(display.name)), color = "black") +
  # geom_point(data = subset(overlaps.mmvl30, rep.dist == 0 & ko4H3_v_YR.padj < 0.05 & repmask.name == "MMVL30-int"), color = "black", size = 1) +
  ...
  annotate("text", x = -3.2, y = 104, label = paste0("n = ", comma_format()(up.peaks)), color = "red", hjust = 0) +
  annotate("text", x = -3.2, y = 96, label = paste0("n = ", comma_format()(down.peaks)), color = "blue", hjust = 0) +
  # annotate("text", x = -3.2, y = 88, label = paste0("MMVL30 n = ", mmvl30.up.peaks), color = "red", hjust = 0) +
  # annotate("text", x = -3.2, y = 80, label = paste0("MMVL30 n = ", mmvl30.down.peaks), color = "blue", hjust = 0) +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
ggsave("ko4H3_v_YR.volcano.png", mp, width = 4, height = 4, dpi = 1200)
...
mp <- ggplot(overlaps.unique.rep.type[ko4H3_v_YR.padj < 0.05 & final.type %in% c("LTR", "DNA", "SINE", "LINE")], aes(final.type, ko4H3_v_YR.l2fc)) +
  geom_hline(yintercept = 0, linetype="dashed", color = "gray") +
  geom_boxplot(outlier.shape = NA, width = 0.5, aes(fill = final.type)) +
  geom_jitter(width = 0.1, size = 0.1, alpha = 0.5) +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
mp
ggsave("ko4H3_v_YR.boxplot.repeat.type.png", mp, width = 4.5, height = 4, dpi = 1200)
